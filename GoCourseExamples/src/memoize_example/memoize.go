...
func main() {
	...
	//found in cache
	fmt.Printf("%4d = %s\n", 1999, RomanForDecimal(1999).(string))
...
func Memoize(function memoizeFunction) memoizeFunction {
	...
	return func(x int, xs ...int) interface{} {
		...
		if value, found := cache[key]; found {
			fmt.Println("Found in cache k ", key)
			return value
		}
		fmt.Println("Executing function with params ", x)
		value := function(x, xs...)
		fmt.Println("Putting in cache")
		cache[key] = value
		return value
	}
}
